...
static void ir_sampling_task(void* arg)
{
    int output;

    // Configura el canal (GPIO PIN) y establece la atenuacion (para voltajes mas altos)
    adc2_config_channel_atten(ADC_CHANNEL_6, ADC_ATTEN_DB_0);

    while(1) {
        
        // La salida esta en el rango [0 - 4095]
        adc2_get_raw(ADC_CHANNEL_6, ADC_WIDTH_BIT_12, &output);

        dist = get_ir_distance(output);
        
        // Resetea si esta a menos de 100 milimetros (10 cm)
        if(dist < 100) reset();
...
static void timer_callback(void* arg)
{
    ...
    int sync_seg = 0, sync_min = 0, sync_dist = 0;
...
void app_main(void)
{
    ...
    */
    ...
    # ifndef CONFIG_IR_RESET_INPUT
        // Crea una tarea para tomar muestras del campo magnetico
        xTaskCreate(&hall_sampling_task, "hall_sampling_task", 2048, NULL, 5, NULL);
    # else
        // Crea una tarea para tomar muestras sensor de infrarojos
        xTaskCreate(&ir_sampling_task, "IR_sampling_task", 2048, NULL, 5, NULL);
    # endif
